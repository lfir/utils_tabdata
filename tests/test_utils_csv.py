...
class TestUtilsCSV(unittest.TestCase):
    ...
    def test_unique_rows(self):
        ...
        self.assertListEqual(
            UtilsCSV().unique_rows(ds, all_columns=True),
            [{"a": "2"}, {"a": "1"}, {"a": "-1"}],
        )
        self.assertListEqual(
            UtilsCSV().unique_rows(ds1, "b"),
            [{"a": "2", "b": "0"}, {"a": "-1", "b": "j"}, {"a": "-1", "b": "k"}],
        )
        self.assertListEqual(
            UtilsCSV().unique_rows(ds2, "a", "b"),
            [{"a": "1", "b": "0"}, {"a": "-1", "b": "j"}, {"a": "-1", "b": "k"}],
        )
    ...
    def test_sort_by_int_in_str(self):
        ...
        self.assertListEqual(
            UtilsCSV().sort_by_int_in_str(pattern, "a", reverse=True), desc_pattern
        )
    ...
    def test_merge_csvs_with_column_order(self):
        self.auxf.merge_csvs(
            "test2", "test.csv", "test1.csv", column_lst=["c", "a", "b"]
        )
