...
from utils_tabdata import scripts, utils_csv
...
class TestScripts(unittest.TestCase):
    ...
    def test_unique_rows(self):
        ...
        self.assertEqual(
            self.auxf.csv_to_dict_list("restsur0.csv"),
            self.auxf.csv_to_dict_list("tsur.csv"),
        )
    ...
    def test_unique_rows_some_columns_only(self):
        ...
        self.assertEqual(
            self.auxf.csv_to_dict_list("restsur1.csv"),
            self.auxf.csv_to_dict_list("tsur.csv"),
        )
        self.assertEqual(result.exit_code, 0)
    ...
    def test_merge_csvs(self):
        result = self.runner.invoke(
            scripts.merge_csvs,
            ["res_merge0", "-f", "mergecsv0.csv", "-f", "mergecsv1.csv"],
        )

        self.assertEqual(
            self.auxf.csv_to_dict_list("res_merge0.csv").__len__(),
    ...
    def test_merge_csvs_some_columns_only(self):
        ...
        self.assertEqual(
            self.auxf.csv_to_dict_list("res_merge1.csv").__len__(),
            self.auxf.csv_to_dict_list("mergecsv0.csv").__len__()
            + self.auxf.csv_to_dict_list("mergecsv1.csv").__len__(),
        )
        self.assertEqual(result.exit_code, 0)
